...
**section3Up1**：
- 满足所有条件可以SSL入场
    - minSSL > maxSSL2 and minSSL2 > maxSSL3（确定上升趋势）
    - `trend == 1`
    - 最近三根K线 qqe柱子是红色 and 拐头向上 and 最近三个qqe柱子的最小值 < -15
    - 最近50根K线一半以上满足：minSSL <= maxSSL2
...
1. (close - 开仓价) / 开仓价 > 0.5% (做多移动止损阈值0.5%可以配置，注意计算用精确值不能用百分数)
    * 之后移动止损位置到 max(原止损价, preLow, minSSL)
    * 后续持续计算止损位：max(原止损价, preLow, minSSL)
2. qqe柱子值 > 30 (-30可配置)
    * 之后移动止损位置到 max(原止损价, preLow, minSSL)
    * 后续持续计算止损位：max(原止损价, preLow, minSSL)
...
2. 指标止盈（可配置，默认关闭）达到下面条件可以止盈（删除该条件）
    * 第3次达到 close > SuperTrend上轨
...
**section3Down1**：
- 满足所有条件可以SSL入场
    - maxSSL < minSSL2 and maxSSL2 < minSSL3（确定为下降趋势）
    - `trend == -1`
    - 最近三根K线 qqe柱子是绿色 and 拐头向下 and 最近三个qqe柱子的最大值 > 15
    - 最近50根K线一半以上满足：maxSSL >= minSSL2
...
1. (开仓价 - close) / 开仓价 > 0.5% (做空移动止损阈值0.5%可以配置，注意计算用精确值不能用百分数)
    * 之后移动止损位置到 min(原止损价, preHigh, maxSSL)
    * 后续持续计算止损位：min(原止损价, preHigh, maxSSL)
2. qqe柱子值 < -30 (-30可配置)
    * 之后移动止损位置到 min(原止损价, preHigh, maxSSL)
    * 后续持续计算止损位：min(原止损价, preHigh, maxSSL)
