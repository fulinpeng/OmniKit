...
// 配置文件路径
const EXCEL_PATH = './input/a.xlsx';
const TEMPLATE_PATH = './input/b.docx';
 ...
 * @returns {Array} 按照 E、B、A、D、C、E 顺序的数据数组
 */
function extractRowData(row) {
  return [
    getColumnValue(row, 'E'),  // 第1个数据：E列
    getColumnValue(row, 'B'),  // 第2个数据：B列
    getColumnValue(row, 'A'),  // 第3个数据：A列
    getColumnValue(row, 'D'),  // 第4个数据：D列
    getColumnValue(row, 'C'),  // 第5个数据：C列
    getColumnValue(row, 'E')   // 第6个数据：E列
  ];
}
...
async function processWordTemplate(templateBuffer, dataArray, fileName) {
  try {
    ...
    // 然后转换为PDF
    await convertWordToPDF(docxPath, fileName);
...
async function convertWordToPDF(docxPath, fileName) {
  ...
  if (!success) {
    console.log('专业转换失败，使用备用方案（mammoth+puppeteer）...');
    // 方案3: 备用方案 - 使用mammoth+puppeteer
    await convertWordToPDFWithPuppeteer(docxPath, fileName);
  }
}
...
async function main() {
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      // 提取数据（按照 E、B、A、D、C、E 顺序）
      const dataArray = extractRowData(row);
      const employeeName = dataArray[1]; // Employee Name 是第二个元素（B列，索引1）
      
      // 生成文件名：A列_F列
      const aColumnValue = row[0]; // A列数据
      const fColumnValue = row[5]; // F列数据
      const baseFileName = `${String(aColumnValue || '').trim()}_${String(fColumnValue || '').trim()}`;
      ...
      // 处理Word文档
      const success = await processWordTemplate(templateBuffer, dataArray, fileName);
      
      if (success) {
        successCount++;
      } else {
        failedCount++;
        failedFiles.push({ fileName, rowNumber: actualRowNumber });
      }
    }
    ...
    if (failedCount > 0) {
      ...
      failedFiles.forEach((item, index) => {
        console.log(`${index + 1}. ${item.fileName} (Excel第${item.rowNumber}行)`);
      });
    }
